...
class DQAgent:
    def __init__(self,
        ...
        # Main DQN
        self.DQN = DQNetwork(self.actions, self.network_input_shape, minibatch_size=self.minibatch_size,
                             learning_rate=self.learning_rate, discount_factor=self.discount_factor,
                             dropout_prob=self.dropout_prob, load_path=self.load_path, logger=self.logger)

        # Target DQN used to generate targets
        self.DQN_target = DQNetwork(self.actions, self.network_input_shape, minibatch_size=self.minibatch_size,
                                    learning_rate=self.learning_rate, discount_factor=self.discount_factor,
                                    dropout_prob=self.dropout_prob, load_path=self.load_path, logger=self.logger)
        # Reset target DQN
    ...
    def add_experience(self, sars):
        ...
        if (len(self.experiences) % 100 == 0) and (len(self.experiences) < self.replay_memory_size) and (
            ...
            t = (time.time() - self.start_time) * (self.replay_memory_size - len(self.experiences)) / (
                        len(self.experiences) + 1)
    ...
    def sample_batch(self):
        ...
        for i in range(self.minibatch_size):
            num = randrange(0, len(self.experiences))
            # while self.experiences[num]['state']
            batch.append(self.experiences[num])
        return np.asarray(batch)

    def slide_batch(self):
        '''
        Samples self.minibatch_size random transitions from the replay memory
        and returns them as a batch.
        :return: a batch of SARS' tuples
        '''
        batch_list=[]
        for i in range(int(len(self.experiences)/self.minibatch_size)):
            batch = []
            for j in range(self.minibatch_size):
                batch.append(self.experiences[j+i*self.minibatch_size])
            batch_list.append(np.asarray(batch))

        if len(self.experiences) % self.minibatch_size != 0 :
            last_range_experiences=self.experiences[-32:-1]
            batch = []
            for experience in last_range_experiences:
